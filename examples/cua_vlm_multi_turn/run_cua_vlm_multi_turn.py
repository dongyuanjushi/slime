...
TRAIN_DATA_PATH = os.environ.get("TRAIN_DATA_PATH", "examples/cua_vlm_multi_turn/no_gdrive_rl_train.jsonl")

def execute():
    ckpt_args = f"--hf-checkpoint {MODEL_NAME} "

    wandb_args = (
        (
            "--use-wandb "
            "--wandb-project slime-dev "
            "--wandb-group cua_vlm_multi_turn "
    ...
    rollout_args = (
        ...
        "--rm-type math "
        "--apply-chat-template "
        "--custom-generate-function-path examples.cua_vlm_multi_turn.qwen3_rollout.generate "
